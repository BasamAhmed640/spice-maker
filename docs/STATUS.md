...
|Loop feedback (live)|an unreadable run's reason now carries the simulator's own line — `sim_output_unreadable: no .raw was written (…); LTspice said: …\BM_REG_BUCK.lib(95): Undefined model "rout_drv"` — and that reason is the text the next turn's prompt quotes, so a turn can tell a syntax error from a missing file (before, both read as "no `.raw`"). The API backend also re-asks **once** inside a turn when the reply is not the required JSON object, quoting the parse error back; a retry that also fails is reported with both attempts named|
