...
Each HTTP extraction request now reports connecting, receiving stream/response bytes,
then stream completion and validation. Only counts and fixed status text reach the UI;
neither API keys nor reasoning text is included. Streaming requests advertise the
correct Accept header. Provider/model selection, reasoning settings and numerical
