...
SPECS: tuple[Spec, ...] = (
    ...
    Spec(
        "ELEC_004", "ELECTRICAL", "DOCUMENTED_LIMIT", "IMPORTANT",
    ...
    Spec(
        "CONN_010", "CONNECTIVITY", "DOCUMENTED_LIMIT", "CRITICAL",
        ...
        "Supply domains", r"Each domain's pins must sit on a net whose declared domain is that domain",
        expression={"op": "pin_connected", "refdes": "U1", "pin": "3V3_IN"},
        signal_refs=("3V3", "1V8"),
    ),
    Spec(
        "CONN_011", "CONNECTIVITY", "DOCUMENTED_LIMIT", "CRITICAL",
        "PERST# is required and must not float; it is active low.",
        "Reset", r"\|Required\|Yes; must not float\|",
        expression={"op": "pin_connected", "refdes": "U1", "pin": "PERST_N"},
        signal_refs=("PERST_N",),
    ),
    Spec(
        "TEMP_012", "TEMPORAL", "DOCUMENTED_LIMIT", "CRITICAL",
        ...
        "Reset", r"Must be held low while \*\*either\*\* domain is outside its allowed range",
        expression={
            "op": "ordering",
    ...
    Spec(
        "TEMP_013", "TEMPORAL", "DOCUMENTED_LIMIT", "CRITICAL",
        "PERST# is released between 1 ms and 100 ms after the last prerequisite becomes valid.",
        "Reset", r"\|Release delay\|At least 1 ms and at most 100 ms after the last prerequisite\|",
    ...
    Spec(
        "CONN_030", "CONNECTIVITY", "DOCUMENTED_LIMIT", "CRITICAL",
        "SMB_CLK and SMB_DAT are open-drain sideband signals pulled up on the 3V3 domain.",
        "Sideband (SMB_CLK, SMB_DAT)", r"\|Pull-up domain\|`3V3`\|",
        expression={
            "op": "pullup_domain",
            "refdes": "U1",
            "pin": "SMB_DAT",
    ...
    Spec(
        "ELEC_031", "ELECTRICAL", "DOCUMENTED_LIMIT", "IMPORTANT",
    ...
    Spec(
        "CONN_032", "CONNECTIVITY", "DOCUMENTED_LIMIT", "CRITICAL",
    ...
    Spec(
        "CONN_040", "CONNECTIVITY", "DOCUMENTED_LIMIT", "IMPORTANT",
        "CLK_REQ# is an active-low open-drain output pulled up on 3V3.",
        "Clock request (CLK_REQ#)", r"\|Pull-up domain\|`3V3`\|",
        expression={
            "op": "pullup_domain",
            "refdes": "U1",
            "pin": "CLK_REQ_N",
    ...
    Spec(
        "SYSTEM_050", "SYSTEM", "ASSUMPTION", "CRITICAL",
    ...
    Spec(
        "FUNC_060", "FUNCTIONAL", "DOCUMENTED_LIMIT", "INFORMATIONAL",
    ...
    Spec(
        "CONN_070", "CONNECTIVITY", "DOCUMENTED_LIMIT", "CRITICAL",
        "A pin on an unpowered domain must not be driven externally (back-powering).",
        "Partial power", r"Any pin on a domain that is unpowered must not be driven externally",
