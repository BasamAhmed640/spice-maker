...
"""
        ...
        # --- LTspice ---------------------------------------------------------
        #: The path appears only after the user browses or saves it in this copy.
        ...
        self.adjustSize()  # let the frame pick its floor…
        self._fit_to_content()  # …then open showing the whole page: no dead space, no scrollbar
    ...
    def _resolved_ltspice(self) -> str:
        """Show only the path explicitly saved in this copy's settings."""
    ...
    def _ltspice_status_text(self) -> str:
        """Describe only the manually chosen or locally saved path."""
